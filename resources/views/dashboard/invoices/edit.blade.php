                    ...
                    <div class="row breadcrumbs-top">
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">  {{ trans('invoice.Edit') }}
                                </li>
                            </ol>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title" id="basic-layout-form"> {{ trans('invoice.Edit') }} </h4>
                                ...
                                <div class="card-content collapse show">
                                    <div class="card-body">
                                        <form class="form"
                                            action="{{ route('invoices.update','test') }}"
                                            method="POST">
                                            {{ method_field('patch') }}
                                            @csrf
                                            <input type="hidden" name="id" value="{{$invoice->id}}" />
                                            <div class="form-body">
                                                <h4 class="form-section"><i class="ft-home"></i> {{ trans('invoice.Edit') }} </h4>
                                                <div class="row">
                                                    <div class="col-md-3">
                                                                ...
                                                                <optgroup label="{{ trans('main-sidebar.patient') }}">
                                                                    @if($patients && $patients -> count() > 0)
                                                                        @foreach($patients as $patient)
                                                                            <option
                                                                                value="{{$patient->id }}" {{ $patient->id == $invoice->patient_id ? ' selected' : '' }}>{{$patient->name}}</option>
                                                                        @endforeach
                                                                    @endif
                                                                </optgroup>
                                                            </select>
                                                                ...
                                                                <optgroup label="{{ trans('main-sidebar.doctors') }}">
                                                                    @if($doctors && $doctors -> count() > 0)
                                                                        @foreach($doctors as $doctor)
                                                                            <option
                                                                                value="{{$doctor->id }}" {{ $doctor->id == $invoice->doctor_id ? ' selected' : '' }}>{{$doctor->name}}</option>
                                                                        @endforeach
                                                                    @endif
                                                                </optgroup>
                                                            </select>
                                                                ...
                                                                <optgroup label="{{ trans('main-sidebar.services') }}">
                                                                    @if($services && $services -> count() > 0)
                                                                        @foreach($services as $service)
                                                                            <option
                                                                                value="{{$service->id }}" {{ $service->id == $invoice->service_id ? ' selected' : '' }}>{{$service->name}}</option>
                                                                        @endforeach
                                                                    @endif
                                                                </optgroup>
                                                            </select>
