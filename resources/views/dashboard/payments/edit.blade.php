                    ...
                    <div class="row breadcrumbs-top">
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active">  {{ trans('payment.Edit') }}
                                </li>
                            </ol>
                        </div>
                    </div>
                        ...
                        <div class="col-md-12">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title" id="basic-layout-form"> {{ trans('payment.Edit') }} </h4>
                                ...
                                <div class="card-content collapse show">
                                    <div class="card-body">
                                        <form class="form"
                                            action="{{ route('payments.update','test') }}"
                                            method="POST">
                                            {{ method_field('patch') }}
                                            @csrf
                                            <input type="hidden" name="id" value="{{$payment->id}}" />
                                            <div class="form-body">
                                                <h4 class="form-section"><i class="ft-home"></i> {{ trans('payment.Edit') }} </h4>
                                                <div class="row">
                                                    <div class="col-md-6">
                                                                ...
                                                                <optgroup label="{{ trans('main-sidebar.patient') }}">
                                                                    @if($patients && $patients -> count() > 0)
                                                                        @foreach($patients as $patient)
                                                                            <option
                                                                                value="{{$patient->id }}" {{ $patient->id == $payment->patient_id ? ' selected' : '' }}>{{$patient->name}}</option>
                                                                        @endforeach
                                                                    @endif
                                                                </optgroup>
                                                            </select>
